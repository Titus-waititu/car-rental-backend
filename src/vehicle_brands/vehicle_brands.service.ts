...
export class VehicleBrandsService {
  constructor(
    @InjectRepository(VehicleBrand) private vehicleBrandsRepository: Repository<VehicleBrand>,
  ) {}
  async create(createVehicleBrandDto: CreateVehicleBrandDto) {
    return await this.vehicleBrandsRepository.save(createVehicleBrandDto).then((vehicleBrand) => {
      return vehicleBrand;
    }
    ).catch((error) => {
  ...
  async findOne(id: number) {
    return await this.vehicleBrandsRepository.findOne({ where: { brand_id: id } }).then((vehicleBrand) => {
      if (!vehicleBrand) {
  ...
  async update(id: number, updateVehicleBrandDto: UpdateVehicleBrandDto) {
    return await this.vehicleBrandsRepository.update(id, updateVehicleBrandDto).then((result) => {
  ...
  async remove(id: number) {
    return await this.vehicleBrandsRepository.delete(id).then((result) => {
      if (result.affected === 0) {
        return `Vehicle brand with ID ${id} not found.`;
      }
